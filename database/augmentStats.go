...
func InsertOrUpdateAugmentStats(tx *sql.Tx, augmentStats *types.AugmentStatsArr) error {
	for _, stat := range *augmentStats {
		...
		_, err := tx.Exec(`
			INSERT INTO tft_augment_stats (match_date, game_version, augment_id, pick, tier, accumulated_placement, frequency)
		...
		`,
			stat.GameDate, stat.GameVersion, stat.AugmentId, stat.Pick, stat.Tier, stat.AccumulatedPlacement, stat.Frequency)
		if err != nil {
			return err
		}
	}
